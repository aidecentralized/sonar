from typing import Dict, Union, List, Any  # Include Any for generic typing
...
class BaseConfig:
    """Base configuration for core SONAR settings."""
...
class AlgoConfig(BaseConfig):
    """Configuration for algorithm-specific settings."""
    ...
    def validate(self) -> None:
        ...
        required_fields = ['algo', 'model', 'model_lr', 'batch_size']
        missing = [field for field in required_fields if not hasattr(self, field)]
        if missing:
            raise ValueError(f"Missing required fields: {missing}")

    def summary(self) -> Dict[str, Any]:
        """Extends BaseConfig's summary with any AlgoConfig specifics."""
        return super().summary()

    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary format with algorithm-specific handling."""
        base_dict = super().to_dict()
        return base_dict

    @staticmethod
    def get_malicious_types(malicious_config_list: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Assign a random malicious type to a single node."""
        return random.choice(malicious_config_list)

fedavg_config = AlgoConfig(
    algo="fedavg",
    model="resnet10",
    model_lr=3e-4,
    batch_size=256,
...
# default_config_list: List[ConfigType] = fedavg_config
