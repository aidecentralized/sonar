...
from configs.algo_config import AlgoConfig, fedavg_config

class SystemConfig:

    def __init__(self, num_users: int, gpu_ids: List[int], algo_configs: List[AlgoConfig], **kwargs):
        self.num_users = num_users
        self.device_ids = self.get_device_ids(num_users, gpu_ids)
        
        algo_config_kwargs = {
            k: kwargs[k] for k in [
                "assignment_method", "seed", "mapping", "distribution"
            ] if k in kwargs
        }
        
        self.algo_configs = self.get_algo_configs(
            num_users,
            algo_configs,
            **algo_config_kwargs
        )

        self.comm = self.setup_communication(kwargs.get("comm_type", "GRPC"), kwargs)
        self.setup_additional_attributes(kwargs)

    def get_device_ids(self, num_users: int, gpus_available: List[int]) -> Dict[str, List[int]]:
        """Assigns GPU device IDs to nodes based on availability."""
        device_ids = {}
        for i in range(num_users + 1):
            gpu_id = gpus_available[i % len(gpus_available)]
    ...
    def get_algo_configs(
        ...
        """Assigns algorithm configurations to nodes based on the specified assignment method."""
        algo_config_map: Dict[str, AlgoConfig] = {"node_0": algo_configs[0]}  # Super-node config
        ...
        elif assignment_method == "mapping" and mapping:
            for i, algo_idx in enumerate(mapping, 1):
                if algo_idx >= len(algo_configs):
                    raise IndexError(f"Algorithm index {algo_idx} is out of range for algo_configs.")
                algo_config_map[f"node_{i}"] = algo_configs[algo_idx]
        elif assignment_method == "distribution" and distribution:
            if seed:
                random.seed(seed)
            node_indices = list(range(1, num_users + 1))
            random.shuffle(node_indices)
            current_index = 0
            for algo_index, num_nodes in distribution.items():
                if algo_index >= len(algo_configs):
                    raise IndexError(f"Algorithm index {algo_index} is out of range for algo_configs.")
    ...
    def setup_communication(self, comm_type: str, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        """Sets up communication configuration for nodes."""
        rank = kwargs.get("rank", None)
        base_port = 50050

        if comm_type == "GRPC":
            address = f"localhost:{base_port + (rank if rank is not None else 0)}"
            return {
                "type": "GRPC",
                "synchronous": kwargs.get("synchronous", True),
                "peer_ids": [f"localhost:{base_port + i}" for i in range(self.num_users + 1)],
                "address": address,  # This node's unique address
                "rank": rank
            }
        elif comm_type == "MPI":
            return {
    ...
    def setup_additional_attributes(self, kwargs: Dict[str, Any]) -> None:
        """Sets additional configuration attributes."""
        for key, value in kwargs.items():
            setattr(self, key, value)

    def summary(self) -> Dict[str, Union[int, Dict[str, List[int]], Dict[str, AlgoConfig]]]:
        """Provides a summary of the system configuration."""
        return {
            "num_users": self.num_users,
            "device_ids": self.device_ids,
            "algo_configs": {k: v.summary() for k, v in self.algo_configs.items()},
            "comm": self.comm
        }

    def get_domain_support(self, base: str, domains: List[Union[int, str]]) -> Dict[str, str]:
        """Assigns domain support across nodes."""
        assert self.num_users % len(domains) == 0, "Number of users must be divisible by the number of domains."
        users_per_domain = self.num_users // len(domains)
        support = {str(i): f"{base}_{domains[i // users_per_domain]}" for i in range(self.num_users + 1)}
        return support


grpc_system_config = SystemConfig(
    num_users=9,
    gpu_ids=[2, 3, 5, 6],
    algo_configs=[fedavg_config], 
    exp_id="static",
    comm_type="GRPC",
    synchronous=True,
    peer_base_port=50048,
    seed=2,
    dset="cifar10",
    dump_dir="sonar_experiments/",  # Use home directory
    dpath="./datasets/imgs/cifar10/",
    samples_per_user=50000 // 9,
