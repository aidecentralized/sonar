...
def copy_source_code(config: dict) -> None:
    ...
    if config["load_existing"]:
        ...
        denylist = ["./__pycache__/", "./.ipynb_checkpoints/", "./expt_dump/", "./helper_scripts/",
                    "./imgs/", "./expt_dump_old/", "./comparison_plots/", "./toy_exp/", "./toy_exp_ml/", "./toy_exp.py",  "./toy_exp_ml.py"
                    '/'.join(path.split('/')[:-1])+'/']
        folders = glob(r'./*/')
...
class LogUtils():
    ...
    def log_min_stats_per_client(self, stats_per_client, round_step, metric):
        self.__log_stats_per_client__(stats_per_client, round_step, metric, is_max=False)

    def __log_stats_per_client__(self, stats_per_client, round_step, metric, is_max=False):
        if is_max:
        ...
        self.summary_file.write(f"============== {minmax} {metric} per client ==============\n")
        for client_idx, (best_round, best_val) in enumerate(zip(best_round_per_client, best_val_per_client)):
            self.summary_file.write(f"Client {client_idx+1} : {best_val} at round {best_round}\n")
