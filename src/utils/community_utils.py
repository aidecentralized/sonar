...
def get_dset_balanced_communities(num_clients, num_communities, num_dset):
    ...
    for dset in range(num_dset):
        clients_id_per_dset.append(list(np.random.permutation(np.arange(1 + dset * clients_per_dset, 1 + (dset+1) * clients_per_dset))))
    
    num_assigned = 0
    communities = {k:[] for k in range(num_communities)}
    while num_assigned < num_clients:
        communities_random_order = {k:v for k,v in  enumerate(np.random.permutation(range(num_communities)))}
        communities_id = sorted(range(num_communities), key=lambda k: (len(communities[k]), communities_random_order[k]))

        current_dset_clients = clients_id_per_dset.pop()
        while len(current_dset_clients) > 0:
            for community_id in communities_id:
                if len(current_dset_clients) == 0: break
...
def get_dset_communities(num_clients, num_dset):
    ...
    for idx in range(num_clients):
        ...
        support[idx+1] = list(np.arange(1 + dset * clients_per_dset, 1 + (dset+1) * clients_per_dset))
    return support