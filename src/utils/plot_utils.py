...
class PlotUtils():
    ...
    def plot_clients_stats_per_round(self, stats, round_step, metric):
        ...
        plt.ylabel(metric)
        if self.with_title:
            plt.title(f"{metric} - {self.config['algo']}-{self.config['dset_name']}-samples per clients: {self.config['samples_per_client']}\nlabel distribution: {self.config['train_label_distribution']}")
        else:
            plt.title(f"{metric}")
    ...
    def plot_clients_avg_stats_per_round(self, stats, round_step, metric):
        ...
        plt.fill_between(x_axis, np.quantile(stats, 0.25, axis=0), 
                         np.quantile(stats, 0.75, axis=0), alpha=0.3)
        plt.xlabel("Round")
        plt.ylabel(metric)
        if self.with_title:
            plt.title(f"avg_{metric} - {self.config['algo']}-{self.config['dset_name']}-clients:{self.config['num_clients']}-samples per clients: {self.config['samples_per_client']}\nlabel distribution: {self.config['train_label_distribution']}")
        else:
            plt.title(f"avg_{metric}")
        plt.savefig(f"{self.plot_dir}/avg_{metric}_per_round.png")
        plt.close()
        
    def plot_avg_clients_weights_heatmap(self, weights_stats, name, x_label, diagonal=None):
        ...
        plt.ylabel("Client")
        if self.with_title:
            plt.title(f"{name} weights - {self.config['algo']}-{self.config['dset_name']}-clients:{self.config['num_clients']}-samples per clients: {self.config['samples_per_client']}\nlabel distribution: {self.config['train_label_distribution']}")
        else:
            plt.title(f"{name} weights")
            
        
        plt.savefig(f"{self.plot_dir}/{str.lower(name).replace(' ', '_')}{('_'+diagonal) if diagonal is not None else ''}.png")
        plt.close()
           
    def plot_clients_weights_heatmap_over_time(self, weights_stats, name, x_label, num_plot=10, 
                                               log_space=False, single_round=True, plot_rounds=None, diagonal=None):
        ...
        if plot_rounds is not None:
            num_plot = len(plot_rounds)
                ...
                plot_rounds = np.logspace(0, np.log10(num_round), num_plot + (0 if single_round else 1), dtype=int)
                plot_rounds = list(dict.fromkeys(plot_rounds)) # remove duplicates
        else:
            plot_rounds = np.linspace(0, num_round-1, num_plot + (0 if single_round else 1), dtype=int)
        ...
        for idx, ax in enumerate(grid):
            ...
            if single_round:
                ...
                weights = weights_stats[:, plot_rounds[idx]:plot_rounds[idx+1]].mean(axis=1)
            ...
            if single_color_bar:
                ...
                im = ax.imshow(weights)#, vmin=vmin, vmax=vmax)
            ax.set_title(f"Round{' '+ str(plot_rounds[idx]) if single_round else f's {plot_rounds[idx]}-{plot_rounds[idx+1]}'}")
        ...
        plt.savefig(f"{self.plot_dir}/{name_str}_weights_over_time_{single_round_str}{diagonal_str}.png")
        plt.close()
    ...
    def plot_clients_score_histogram(self, clients_stats, name):
        ...
        f, axs = plt.subplots(n_row, n_col, figsize=(4 * n_row, 4 * n_col), sharex=True, sharey=True)        
        ...
        if self.with_title:
            f.suptitle(f"{name} -- {self.config['algo']}-{self.config['dset_name']}-clients:{self.config['num_clients']}-samples per clients: {self.config['samples_per_client']}\nlabel distribution: {self.config['train_label_distribution']})")
        else:
            f.suptitle(f"{name}")
    ...
    def plot_score_histogram_evolution_per_clients(self, clients_stats, name, num_plot_per_clients=10, round_log=False, single_round=True):
        # Histogram per clients of mean clients score over rounds
        
        num_round = clients_stats.shape[1]
        if round_log:
                plot_rounds = np.logspace(1, np.log10(num_round), num_plot_per_clients + (0 if single_round else 1), dtype=int)
                plot_rounds = list(dict.fromkeys(plot_rounds)) # remove duplicates
        else:
            plot_rounds = np.linspace(1, num_round+1, num_plot_per_clients + (0 if single_round else 1), dtype=int)

        num_client = clients_stats.shape[0]
        n_col = num_plot_per_clients
        n_row = num_client
        f, axs = plt.subplots(n_row, n_col, figsize=(3 * n_row + 3, 3 * n_col+3), sharex=True, sharey=True)        
        for plt_idx in range(n_col * n_row):
            ...
            if single_round:
                ...
                weights = clients_stats[client_idx, round_start-1:round_end-1].mean(axis=0)     
                ax.set_title(f"Client {client_idx + 1}, rounds {round_start}-{round_end}")
        ...
        f.text(0.5,0.04, name, ha="center", va="center")
        f.text(0.05,0.5, "Clients", ha="center", va="center", rotation=90)
        plt.savefig(f"{self.plot_dir}/{name.lower().replace(' ', '_')}_evolution_hist.png")
        plt.close()
    ...
    def plot_client_collaboration_evolution(self, collab_weights_stats, client_idx, dir=""): 
        ...
        for client in range(client_weight_per_rounds.shape[0]):
            plt.plot(x_axis, client_weight_per_rounds[client], label=f"client{client+1}")
        plt.legend()
        plt.xlabel("Round")
        plt.ylabel("Collaboration weight")
        if self.with_title:
            plt.title(f"Collaboration weight of client {client_idx + 1}- {self.config['algo']}-{self.config['dset_name']}-samples per clients: {self.config['samples_per_client']}\nlabel distribution: {self.config['train_label_distribution']}")
        else:
    ...
    def plot_clients_weights_evolution_one_plot(self, weights_stats, name): 
        ...
        # Plot grid of subplot each grid show evolution of client weights over round
        fig, ax = plt.subplots(grid_size, grid_size, figsize=(12, 10), sharex = True, sharey = True)
        ...
        if self.with_title:
            fig.suptitle(f"{name} weight of client - {self.config['algo']}-{self.config['dset_name']}-samples per clients: {self.config['samples_per_client']}\nlabel distribution: {self.config['train_label_distribution']}")
        else:
            fig.suptitle(f"{name} weight of client")
        plt.savefig(f"{self.plot_dir}/{str.lower(name).replace(' ', '_')}_weights_evolution.png")
        plt.close()
    ...
    def plot_experiments_stats(self, stats):
        ...
        single_score_stats = [
            ...
            "test_acc", "test_acc_before_training", "test_acc_after_training",
            "lowest_inv_loss",
            "pseudo grad norm"]
        ...
        for stat_name, stat in stats.items():
            if stat_name == "round_step":
                ...
                self.plot_avg_clients_weights_heatmap(stat, stat_name, x_label=x_label, diagonal="zero")

                self.plot_clients_weights_heatmap_over_time(stat, stat_name, single_round=True, x_label=x_label)#, plot_rounds=[1, 2, 3, 4, 5, 20, 40, 60, 80, 100])
                self.plot_clients_weights_heatmap_over_time(stat, stat_name, single_round=False, x_label=x_label)
                self.plot_clients_weights_heatmap_over_time(stat, stat_name, single_round=False, x_label=x_label, diagonal="zero")
                ...
                if isinstance(self.config["dset"], dict) and stat_name in sim_metric:
                    clients_per_community = self.config["num_clients"] // len(set(self.config["dset"].values()))
                    self.plot_similarity_metric_accuracy(stat, 1, True, clients_per_community, stat_name)
                    self.plot_similarity_metric_accuracy(stat, clients_per_community-1, True, clients_per_community, stat_name)
    ...
    def plot_similarity_metric_accuracy(self, similarity_metric, top_k, is_lowest, clients_per_community, name):
        ...
        for i in range(n_rounds):
            np.fill_diagonal(similarity_metric[:, i,:], -np.inf if is_lowest else np.inf)
        
        commu_prediction = (similarity_metric.argsort(axis=2)//clients_per_community)[:,:,1:]

        own_cluster_prediction = commu_prediction[:,:,:top_k]
        top_true = np.repeat(np.arange(num_commu), clients_per_community).reshape((1,-1)).T
        ...
        cluster_accuracy_std = np.std(np.mean(own_cluster_prediction == true_cluster, axis=2), axis=0)

        plt.plot(cluster_accuracy)
        plt.fill_between(np.arange(n_rounds), cluster_accuracy - cluster_accuracy_std, cluster_accuracy + cluster_accuracy_std, alpha=0.2)
        plt.legend()
