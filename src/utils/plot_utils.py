...
class PlotUtils():
    def __init__(self, config, with_title=True) -> None:
        ...
        if not os.path.exists(
                self.plot_dir) or not os.path.isdir(
                self.plot_dir):
    ...
    def plot_clients_weights_heatmap_over_time(
        ...
        if plot_rounds is not None:
            ...
            plot_rounds = np.logspace(0,
                                      np.log10(num_round),
                                      num_plot + (0 if single_round else 1),
                                      dtype=int)
    ...
    def plot_clients_score_histogram(self, clients_stats, name):
        ...
        f, axs = plt.subplots(n_row, n_col, figsize=(
            4 * n_row, 4 * n_col), sharex=True, sharey=True)
        ...
        plt.savefig(
            f"{self.plot_dir}/{name.lower().replace(' ', '_')}_hist.png")
        plt.close()
    ...
    def plot_score_histogram_evolution_per_clients(
        ...
        if round_log:
            plot_rounds = np.logspace(1, np.log10(
                num_round), num_plot_per_clients + (0 if single_round else 1), dtype=int)
            plot_rounds = list(dict.fromkeys(plot_rounds))  # remove duplicates
        else:
            plot_rounds = np.linspace(1, num_round +
                                      1, num_plot_per_clients +
                                      (0 if single_round else 1), dtype=int)
        ...
        f, axs = plt.subplots(
            n_row, n_col, figsize=(
                3 * n_row + 3, 3 * n_col + 3), sharex=True, sharey=True)
        for plt_idx in range(n_col * n_row):
            ...
            if single_round:
                ...
                weights = clients_stats[client_idx,
                                        round_start - 1:round_end - 1].mean(axis=0)
                ax.set_title(
    ...
    def plot_clients_collaboration_evolution_separate_plots(
            self, collab_weights_stats):
        dir = "collab_weights_evol"
        path = self.plot_dir + "/" + dir
        if not os.path.exists(path) or not os.path.isdir(path):
            os.makedirs(path)
        for client in range(collab_weights_stats.shape[0]):
            self.plot_client_collaboration_evolution(
                collab_weights_stats, client, dir)
    ...
    def plot_client_collaboration_evolution(
        ...
        for client in range(client_weight_per_rounds.shape[0]):
            plt.plot(
                x_axis,
                client_weight_per_rounds[client],
    ...
    def plot_clients_weights_evolution_one_plot(self, weights_stats, name):
        ...
        # round
        fig, ax = plt.subplots(
            grid_size, grid_size, figsize=(
                12, 10), sharex=True, sharey=True)
        ...
        for client in range(num_clients):
            ...
            for c in range(num_score):
                ax[row, col].plot(weights_stats[client, :, c],
                                  label=f"client {c+1}")
    ...
    def plot_experiments_stats(self, stats):
        ...
        for stat_name, stat in stats.items():
            if stat_name == "round_step":
                ...
                self.plot_clients_avg_stats_per_round(
                    stat, round_step, stat_name)
                ...
                self.plot_avg_clients_weights_heatmap(
                    stat, stat_name, x_label=x_label)
                self.plot_avg_clients_weights_heatmap(
                ...
                self.plot_clients_weights_heatmap_over_time(
                    ...
                    stat_name,
                    single_round=True,
                    x_label=x_label)  # , plot_rounds=[1, 2, 3, 4, 5, 20, 40, 60, 80, 100])
                self.plot_clients_weights_heatmap_over_time(
                    stat, stat_name, single_round=False, x_label=x_label)
                self.plot_clients_weights_heatmap_over_time(
                    stat, stat_name, single_round=False, x_label=x_label, diagonal="zero")
                self.plot_clients_weights_evolution_one_plot(stat, stat_name)

                if isinstance(
                        self.config["dset"],
                        dict) and stat_name in sim_metric:
                    ...
                    self.plot_similarity_metric_accuracy(
                        stat, clients_per_community - 1, True, clients_per_community, stat_name)
    ...
    def plot_training_distribution(self, split_labels, dset_name, clients_id):
        ...
        for clt_idx, clt_id in enumerate(clients_id):
            labels = [np.where(split_labels[clt_idx] == i)
                      [0].shape[0] for i in cls]
        ...
        plt.savefig(
            f"{self.plot_dir}{dset_name}_training_label_distribution.png")
        plt.close()
    ...
    def plot_similarity_metric_accuracy(
        ...
        top_true = np.repeat(
            np.arange(num_commu), clients_per_community).reshape(
            (1, -1)).T
        true_cluster = np.repeat(top_true, top_k, axis=1)[:, np.newaxis, :]

        cluster_accuracy = np.mean(
            own_cluster_prediction == true_cluster, axis=(0, 2))
        cluster_accuracy_std = np.std(
            np.mean(
                own_cluster_prediction == true_cluster,
                axis=2),
            axis=0)
