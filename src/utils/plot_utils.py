...
class PlotUtils:
    ...
    def log_nx_graph_image(self, iteration: int, directory: str, output_dir: str| None = None):
        """
        Log the networkx directed graph as an image with non-overlapping edges.
        """

        adjacency_list = self.create_adjacency_list(directory)
        graph = nx.DiGraph(adjacency_list)
        # Generate a layout with more spacing
        if self.nx_layout is None:
            self.nx_layout = nx.shell_layout(graph, nlist=[range(1, self.num_users+1)]) # Increase spacing with `k`
        ...
        if output_dir:
            ...
            plt.savefig(f"{self.plot_dir}/graph_{iteration}.png", format="png")
    ...
    def log_nx_graph_edge_weights(self, graph: Graph, iteration: int, directory: str | None = None):
        ...
        if self.nx_layout is None:
            self.nx_layout = nx.shell_layout(graph , nlist=[range(1, self.num_users+1)]) 
        pos = self.nx_layout
        ...
        if directory:
            ...
            plt.savefig(f"{self.plot_dir}/weighted_graph_{iteration}.png", format="png")

        plt.close()

    def combine_graphs_with_edge_frequency(self, directory: str, iteration: int, output_dir: str | None = None):
        """
        ...
        self.log_nx_graph_edge_weights(G, iteration)

        # create video of the graphs
        if output_dir:
            self.create_video(output_dir)
            self.create_heatmap(edge_frequency, output_dir)
        else:
            self.create_video(self.plot_dir)
            self.create_heatmap(edge_frequency, self.plot_dir)
    ...
    def create_heatmap(self, edge_frequency, directory: str):
        ...
        edge_frequency_matrix = np.zeros((self.num_users+1, self.num_users+1))
        ...
        plt.title("Edge Frequency Heatmap")
        plt.xlabel("Node")
        plt.ylabel("Node")
        plt.xticks(range(1, self.num_users+1))
        plt.yticks(range(1, self.num_users+1))
