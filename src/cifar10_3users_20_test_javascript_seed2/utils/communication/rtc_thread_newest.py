...
class RTCCommUtils(CommunicationInterface):
    ...
    def setup_data_channel(self, channel: RTCDataChannel, peer_rank: int):
        ...
        async def send_messages():
            self.logger.info("[WebRTC] send_messages() started")
            while True:
                try:
                    message = self.send_queue.get_nowait()  # Get message from the queue
                    self.logger.info(f"[WebRTC] Popped message from send queue for peer {peer_rank}: {message}")
                    ...
                    self.logger.info(f"[WebRTC] Sent: {message}")
                except Empty:
                    self.logger.info("[WebRTC] No message in send_queue, sleeping...")
                    await asyncio.sleep(0.05)
                      # Reduced sleep duration for faster checks
        @channel.on("message")
        def on_message(message):
            self.logger.info(f"[WebRTC] Received message from peer {peer_rank}: {message}")
            self.message_queue.put([peer_rank, message])
        ...
        while not self.stop_workers:
            try:
                ...
                try:
                    ...
                    self.logger.info(f"Node {self.rank} worker finished processing connection to {target_rank}")
    ...
    def create_session(self, max_clients: int) -> bool:
        try:
            ...
            if data['type'] == 'session_created':
                ...
                self.logger.info(f"Share this session code with other clients: {self.session_id}")
                return True
            else:
    ...
    def initialize(self, create: bool = False, session_id: str = None, max_clients: int = None) -> bool:
        ...
        """
        self.logger.info("Initializing rtc node")
        # Set the variables to class variables

        async def async_run():
            self.logger.info("inside async run")
            max_clients = self.size
            session_id = self.session_id
            self.logger.info("before initializing, rank is", self.rank)
            ...
            try:
                ...
                self.logger.info("Sleeping async to keep connections alive")
                await asyncio.sleep(120)
                self.logger.info("async sleep complete")
                return True
            ...
            while time.time() - start_time < timeout:
                try:
                    ...
                    if data['type'] == 'topology':
                        self.logger.info("Received topology")
                        self.handle_topology(data)
                    elif data['type'] == 'signal':
                        self.logger.info("Received signaling message")
    ...
    def handle_topology(self, data):
        ...
        self.logger.info("Expected connections:", self.expected_connections)
    ...
    def handle_data_channel_message(self, peer_rank: int, message: str):
        try:
            data = json.loads(message)
            msg_type = data["type"]
            self.logger.info(f"handle_data_channel_message from peer {peer_rank}: {message}")
            ...
            if msg_type == "round_update":
                ...
                request_id = data["request_id"]
                self.logger.info(f"Received weights response from peer {peer_rank}: request_id {request_id}")
    ...
    def get_peer_weights(self, peer_rank: int, max_retries: int = 3) -> Optional[np.ndarray]:
        for attempt in range(max_retries):
            try:
                ...
                self.logger.info(f"[Main Thread] Received from message queue: {message}")

                if (message):
                    self.handle_data_channel_message(message[0], message[1])
                else:
                    self.logger.warning(f"message_queue empty, attempt {attempt + 1}")
    ...
    def send(self, dest: str | int, data: Any):
        self.logger.info("RTCCommUtils send called")

    def broadcast(self, data: Any):
        self.logger.info("RTCCommUtils broadcast called")

    def all_gather(self):
        """
        This function is used to gather data from all the nodes.
        """
        self.logger.info("RTCCommUtils all_gather called")
