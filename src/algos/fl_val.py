...
class FedValClient(BaseFedAvgClient):
    ...
    def get_collaborator_similarity(self, reprs_dict):
        ...
        for client_id, model in reprs_dict.items():
            ...
            collab_weights[client_id] = self.evaluate_model(self.model, self.dloader, self.loss_fn, self.device)
        self.set_model_weights(own)
            
        return collab_weights
    
    def select_top_k(self, collab_weights, k):
        collab_similarity = {key: value for key, value in collab_weights.items() if key in self.communities[self.node_id]}
    ...
    def run_protocol(self):
        ...
        for round in range(start_round, total_rounds):
            ...
            collab_weights_dict, proba_dist = self.select_top_k(sim_dict, num_collaborator)
