...
class DisPFLClient(BaseClient):
    ...
    def calculate_sparsities(self, params, tabu=[], distribution="ERK", sparse=0.5):
        ...
        if distribution == "uniform":
            ...
            # With the valid epsilon, we can set sparsities of the remaning
            # layers.
            for name in params:
                if name in dense_layers:
                    spasities[name] = 0
    ...
    def _benefit_choose(
        ...
        if client_num_in_total == client_num_per_round:
            # If one can communicate with all others and there is no bandwidth
            # limit
            client_indexes = [
    ...
    def run_protocol(self):
        ...
        for round in range(start_epochs, total_epochs):
            ...
            print(
                "Node{}: local mask change {}/{}".format(
                    self.node_id, dist_locals[self.index], total_dis
                )
            )
            ...
            if active_ths_rnd[self.index] == 1:
                new_repr, w_per_globals[self.index] = self.aggregate(
                    nei_indexs, weights_lstrnd, masks_lstrnd
                )
            else:
