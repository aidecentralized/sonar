...
class CentralizedCLient(BaseClient):
    ...
    def run_protocol(self):      
        ...
        self.comm_utils.send(dest=self.server_node, data=(self.central_client, self.train_dset), tag=self.tag.SEND_DATA)
        
        global_dloader = None
        if self.is_central_client:
            global_dset = self.comm_utils.receive(node_ids=self.server_node, tag=self.tag.SHARE_DATA)
            #train_dset = Subset(self.dset_obj.train_dset, train_indices)
            batch_size = self.config["batch_size"]
            global_dloader = DataLoader(global_dset, batch_size=batch_size, shuffle=True)
        ...
        for round in range(start_round, total_rounds):
            ...
            if self.is_central_client:
                ...
                self.comm_utils.send(dest=self.server_node, data=(self.communities[self.node_id], global_model), tag=self.tag.SEND_MODEL)
                self.comm_utils.receive(node_ids=self.server_node, tag=self.tag.SHARE_MODEL)
            else:
                round_stats["train_loss"], round_stats["train_acc"] = 0.0, 0.0
                global_model = self.comm_utils.receive(node_ids=self.server_node, tag=self.tag.SHARE_MODEL)
...
class CentralizedServer(BaseServer):
    ...
    def run_protocol(self):
        ...
        for central_client, dsets in dset_per_central_client.items():
            ...
            self.comm_utils.send(dest=central_client, data=dset, tag=self.tag.SHARE_DATA)
        ...
        for round in range(start_round, total_round):
            ...
            central_models = self.comm_utils.receive(node_ids=list(dset_per_central_client.keys()), tag=self.tag.SEND_MODEL)
            for clients, model in central_models:
                for client in clients:
                    self.comm_utils.send(dest=client, data=model, tag=self.tag.SHARE_MODEL)
