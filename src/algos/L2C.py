...
class L2CClient(BaseFedAvgClient):
    ...
    def init_collab_weights(self):
        ...
        self.alpha_optim = optim.Adam([self.alpha], lr=self.config["alpha_lr"], weight_decay=self.config["alpha_weight_decay"])
    ...
    def filter_out_worse_neighbors(self, num_neighbors_to_keep):
        ...
        if num_neighbors_to_keep >= self.alpha.shape[0]:
            raise ValueError("Number of neighbors to keep is greater than the number of neighbors")
        ...
        if num_neighbors_to_keep <= 0:
           ...
           self.alpha = torch.ones(1, requires_grad=True)
            ...
            own_mask[own_idx] = 0
            remaining_neighbors_idx = torch.topk(self.alpha[own_mask], k=num_neighbors_to_keep, largest=True)[1].tolist()
            # Fix the shift created by masking own id
            remaining_neighbors_idx = [idx + 1 if idx >= own_idx else idx for idx in remaining_neighbors_idx]
            remaining_neighbors_idx.append(own_idx)
   
            # Get a list of id and idx of neighbors to keep
            remaining_neighbors_id_idx = [(id, idx) for id, idx in self.neighbors_id_to_idx.items() if idx in remaining_neighbors_idx]
            
            # Sort the list by idx (as in alpha)
            remaining_neighbors_id_idx = sorted(remaining_neighbors_id_idx, key=lambda x: x[1])
            ...
            self.neighbors_id_to_idx = {id:new_idx for new_idx, (id, _) in enumerate(remaining_neighbors_id_idx)}
    ...
    def learn_collab_weights(self, models_update_wts):
        ...
        print("Node {}'s updating alpha loss:{}, acc: {}".format(self.node_id, alpha_loss,acc))
    ...
    def get_model_weights_without_keys_to_ignore(self):
        return self.model_utils.filter_model_weights(self.get_model_weights(), self.model_keys_to_ignore)
    ...
    def get_representation(self):
        if self.sharing_mode == "updates":
            ...
            return self.model_utils.substract_model_weights(self.prev_model, self.get_model_weights_without_keys_to_ignore())
        elif self.sharing_mode == "weights":
    ...
    def run_protocol(self):
        ...
        for round in range(start_round, total_rounds):
            ...
            # Collab weights compted in previous round are used in current round
            ...
            print("node {} weight: {}".format(self.node_id, self.collab_weights))  
...
class L2CServer(BaseFedAvgServer):
    ...
    def single_round(self):
        ...
        round_stats = self.comm_utils.wait_for_all_clients(self.clients, self.tag.ROUND_STATS) 
