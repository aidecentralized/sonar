...
# Encode conv layers and batch norm layers, last linear layer is ignored by default
class ModelEncoder(nn.Module):
    ...
    def init_encoder_weights(self, model_dict):
        ...
        bn_weights_keys = [key for key in keys if "bn" in key and ("weight" in key or "bias" in key)]
        ...
        for key in conv_keys:
            model = nn.Sequential(
                nn.Linear(in_features=math.prod(model_dict[key].shape[1:]),out_features=10,bias=True),
        ...
        for key in bn_weights_keys:
            model = nn.Sequential(
                nn.Linear(in_features=model_dict[key].shape[0],out_features=10,bias=True),
                nn.Linear(in_features=10,out_features=5,bias=True),
            )
            self.encoder_weights[self.weight_key_converter(key)] = model
    ...
    def forward(self, model_dict):        
        ...
        for key in self.ordered_keys:
            wts = model_dict[key]
            if "conv" in key:
                encoder_outs.append(self.encoder_weights[self.weight_key_converter(key)](wts.view(wts.shape[0], -1)).flatten())
            else:
                encoder_outs.append(self.encoder_weights[self.weight_key_converter(key)](wts))
...
class MetaL2CClient(BaseFedAvgClient):
    ...
    def __init__(self, config) -> None:
        ...
        self.encoder_optim = optim.SGD(self.encoder.parameters(), lr=self.config["alpha_lr"])
    ...
    def get_representation(self):
        repr = self.model_utils.substract_model_weights(self.get_model_weights(), self.model_init)
        return repr
    
    def get_knowledge_sharing_artifact(self):
        if self.sharing_mode == "updates":
            return self.model_utils.substract_model_weights(self.prev_model, self.get_model_weights())
    ...
    def learn_collab_weights(self, models_update_wts, collab_weights_tensor_dict):
        ...
        for data, target in self.val_dloader:
            ...
            for cw_id in collab_weights_tensor_dict.keys():
                ...
                # TODO Probably better way of doing this with a single backward pass
        ...
        print("Node {}'s learning collab weight, loss:{}, acc: {}".format(self.node_id, val_loss, val_acc))
    ...
    def filter_out_worse_neighbors(self, num_neighbors_to_keep, collab_weights_dict):
        ...
        if num_neighbors_to_keep <= 0:
           self.neighbors_ids = []
            ...
            self.neighbors_ids = [id for id, _ in collab_weights_dict[:num_neighbors_to_keep]]
    ...
    def run_protocol(self):
        ...
        for round in range(start_round, total_rounds):
            ...
            if round == self.config["T_0"]:
                self.filter_out_worse_neighbors(self.config["target_clients_after_T_0"], collab_weights_dict)
            ...
            avg_alpha = self.comm_utils.wait_for_signal(src=self.server_node, tag=self.tag.ROUND_START)
            ...
            # Send the knowledge sharing artifact at the same time to save communication rounds
            ...
            self.comm_utils.send_signal(dest=self.server_node, data=(repr, ks_artifact), tag=self.tag.REPR_ADVERT)
            ...
            # Knowledge sharing artifact are received together with the representations to save communication rounds
            ...
            self.comm_utils.send_signal(dest=self.server_node, data=(round_stats, alpha), tag=self.tag.ROUND_STATS)
...
class MetaL2CServer(BaseFedAvgServer):
    ...
    def single_round(self, avg_alpha):
        ...
        for client_node in self.clients:
            self.comm_utils.send_signal(dest=client_node, data=avg_alpha, tag=self.tag.ROUND_START)
        ...
        round_stats_and_alphas = self.comm_utils.wait_for_all_clients(self.clients, self.tag.ROUND_STATS) 
