...
class FedStaticClient(BaseFedAvgClient):
    ...
    def get_collaborator_weights(self, reprs_dict: Dict[int, Any], rnd: int) -> Dict[int, float]:
        ...
        within_community_sampling = self.config.get(
            "within_community_sampling", 1)
        ...
        for idx in selected_ids:
            own_aggr_weight = self.config.get(
                "own_aggr_weight", 1 / len(selected_ids))
    ...
    def _apply_aggr_weight_strategy(self, weight: float, rnd: int, total_rounds: int) -> float:
        ...
        if strategy is not None:
            ...
            if strategy == "linear":
                ...
                weight = 1 - (init_weight + (target_weight -
                              init_weight) * (min(1, rnd / target_round)))
            elif strategy == "log":
                alpha = 0.05
                weight = 1 - (init_weight + (target_weight - init_weight) *
    ...
    def run_protocol(self) -> None:
        ...
        for rnd in range(start_round, total_rounds):
            ...
            collab_weights_dict = self.get_collaborator_weights(
                reprs_dict, rnd)
            models_wts = reprs_dict

            layers_to_ignore = self.model_keys_to_ignore
            active_collab = {
                k for k, v in collab_weights_dict.items() if v > 0}
            inter_commu_last_layer_to_aggr = self.config.get(
                "inter_commu_layer", None)
