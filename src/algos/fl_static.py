...
class FedStaticClient(BaseFedAvgClient):
    ...
    def get_collaborator_weights(self, reprs_dict, round):
        ...
        if p_within_decay is not None:
            if p_within_decay == "linear_inc":
                within_community_sampling = within_community_sampling * (
                    round / total_rounds
                )
            elif p_within_decay == "linear_dec":
                within_community_sampling = within_community_sampling * (
                    1 - round / total_rounds
                )
            elif p_within_decay == "exp_inc":
                # Alpha scaled so that it goes from p to (1-p) in R rounds
                alpha = np.log(
                    (1 - within_community_sampling) / within_community_sampling
                )
                within_community_sampling = within_community_sampling * np.exp(
                    alpha * round / total_rounds
                )
            elif p_within_decay == "exp_dec":
                # Alpha scaled so that it goes from p to (1-p) in R rounds
                alpha = np.log(
                    within_community_sampling / (1 - within_community_sampling)
                )
                within_community_sampling = within_community_sampling * np.exp(
                    -alpha * round / total_rounds
                )
                ...
                within_community_sampling = within_community_sampling * np.log2(
                    1 + alpha * round / total_rounds
                )

        algo = self.config["algo"]
        if algo == "random":
            topology = RandomTopology()
            selected_ids = topology.get_selected_ids(
        ...
        for idx in selected_ids:
            ...
            if aggr_weight_strategy is not None:
                ...
                if aggr_weight_strategy == "linear":
                    ...
                    own_aggr_weight = 1 - (
                        init_weight
                        + (target_weight - init_weight) * (min(1, round / target_round))
                    )
                    ...
                    own_aggr_weight = 1 - (
                        ...
                        * (
                            np.log(alpha * (round / total_rounds) + 1)
                            / np.log(alpha + 1)
                        )
                    )
                else:
                    raise ValueError(
                        f"Aggregation weight strategy {aggr_weight_strategy} not implemented"
                    )
    ...
    def run_protocol(self):
        ...
        for round in range(start_round, total_rounds):
            ...
            # If partial merging is on and some client selected client is
            # outside the community, ignore layers after specified layer
