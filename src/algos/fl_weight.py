...
class FedWeightClient(BaseFedAvgClient):
    ...
    def cal_similarity(self, representations :Dict[int, OrderedDict[str, Tensor]]):
        ...
        if similarity == "CosineSimilarity":
            ...
            sim_func = lambda x,y:((x-y)**2).sum(axis=0)
        else:
        ...
        sim_dict = {id: float(sim_func(model1,self.flatten_repr(repr))) for id, repr in representations.items()}
    ...
    def get_k_higest_sim(self, sim_dict, k):
        ...
        collaborator_dict = {client_id:(1/len(selected_clients) if client_id in selected_clients else 0) for client_id in sim_dict.keys()}
    ...
    def run_protocol(self):
        ...
        for round in range(start_round, total_rounds):
            ...
            if self.with_sim_consensus:
                self.comm_utils.send_signal(dest=self.server_node, data=reprs_similarity, tag=self.tag.C_SELECTION)
                reprs_similarity = self.comm_utils.wait_for_signal(src=self.server_node, tag=self.tag.KNLDG_SHARE)
                
            #Select K clients that have the highest similarity compared to their own node
            collab_weights_dict = self.get_k_higest_sim(reprs_similarity, k=self.config["target_clients"])
...
class FedWeightServer(BaseFedAvgServer):
    ...
    def single_round(self):
        ...
        if self.with_sim_consensus:
            sim_dicts = self.comm_utils.wait_for_all_clients(self.clients, self.tag.C_SELECTION)
            sim_dicts = {k:v for k,v in enumerate(sim_dicts, 1)}

            for client_node in self.clients:
                self.comm_utils.send_signal(dest=client_node, data=sim_dicts, tag=self.tag.KNLDG_SHARE)
        ...
        self.log_utils.log_console(f"Round test_acc_before_training {[stats['test_acc_before_training'] for stats in round_stats]}")
        self.log_utils.log_console(f"Round test_acc_after_training {[stats['test_acc_after_training'] for stats in round_stats]}")
