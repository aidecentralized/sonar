...
from utils.data_utils import random_samples, filter_by_class, get_dataset, non_iid_balanced, balanced_subset, CacheDataset, TransformDataset
from utils.log_utils import LogUtils
from utils.model_utils import ModelUtils
from utils.community_utils import get_random_communities, get_dset_balanced_communities, get_dset_communities
...
class BaseNode(ABC):
    ...
    def set_model_parameters(self, config):
        ...
        self.model = self.model_utils.get_model(config["model"],
                                                self.dset, 
                                                self.device, 
                                                self.device_ids,
        ...
        self.optim = optim(self.model.parameters(), lr=config["model_lr"], weight_decay=config.get("weight_decay", 0))
        self.loss_fn = torch.nn.CrossEntropyLoss()

    def set_shared_exp_parameters(self, config):       
        
        if self.node_id != 0:
            community_type, number_of_communities = config.get("community_type", None), config.get("num_communities", 1)
            num_dset = 1 if not isinstance(config["dset"], dict) else len(set(config["dset"].values()))
            if community_type is not None and community_type == "dataset":
                ...
                self.communities = get_random_communities(config["num_clients"], number_of_communities)
            elif community_type == "balanced":
                num_dset = 1 if not isinstance(config["dset"], dict) else len(set(config["dset"].values()))
                # Assume clients ordered by dataset and same number of clients per dataset
                self.communities = get_dset_balanced_communities(config["num_clients"], number_of_communities, num_dset)
            else:
...
class BaseClient(BaseNode):
    ...
    def set_parameters(self, config):
        ...
        # Set same seed for all clients for data distribution and shared parameters so that all clients have the same data splits and shared params 
        seed = config["seed"] 
        ...
        if isinstance(config["dset"], dict):
            seed += sorted(config["dset"].values()).index(config["dset"][str(self.node_id)])
            torch.manual_seed(seed); random.seed(seed); numpy.random.seed(seed)

        self.set_data_parameters(config) 
        # Number of random operation not the same across clients with different datasets
        # Random state is not expected to be the same across clients after this point
        ...
        # If leader_mode on all node need to choose same random node at each round
    ...
    def set_data_parameters(self, config):
        ...
        if isinstance(config["dset"], dict):
            def is_same_dest(dset):
                ...
                # To avoid having exactly same original dataset (without considering transformation) on multiple clients
            ...
            clients_with_same_dset = sorted([int(k) for k, v in config["dset"].items() if is_same_dest(v)])        
        else:
            clients_with_same_dset = list(range(1, config["num_clients"]+1))
        client_idx = clients_with_same_dset.index(self.node_id)

        cls_prior = None
        # If iid, each client has random samples from the whole dataset (no overlap between clients)
        if config["train_label_distribution"] == "iid":
            indices = numpy.random.permutation(len(train_dset))
            train_indices = indices[client_idx*samples_per_client:(client_idx+1)*samples_per_client]
            train_dset = Subset(train_dset, train_indices)
            classes = list(set([train_dset[i][1] for i in range(len(train_dset))]))
        # If non_iid, each client get random samples from its support classes (mulitple clients might have same images)
            ...
            train_dset, sel_indices = random_samples(support_classes_dataset, samples_per_client)
            ...
            if config["train_label_distribution"] == "inter_domain_non_iid":
                ...
                if isinstance(config["dset"], dict) and isinstance(config["dset"], dict):
                    cls_priors = []
                    dsets = list(config["dset"].values())
                    for _ in dsets:
                        n_cls = self.dset_obj.NUM_CLS 
                        cls_priors.append(np.random.dirichlet(alpha=[alpha]*n_cls,size=len(clients_with_same_dset)))
                    cls_prior = cls_priors[dsets.index(self.dset)]
            train_y, train_idx_split, cls_prior = non_iid_balanced(self.dset_obj, len(clients_with_same_dset), samples_per_client, alpha, cls_priors=cls_prior, is_train=True)
            ...
            raise ValueError("Unknown train label distribution: {}.".format(config["train_label_distribution"]))
        ...
        if val_prop > 0:
            ...
            train_dset, val_dset = torch.utils.data.random_split(train_dset, [train_size, val_size])
        ...
        if config["test_label_distribution"] == "iid":
            pass
        # If non_iid, each clients ge the whole test set for each of its support classes
            ...
            test_y, test_idx_split, _ = non_iid_balanced(self.dset_obj, len(clients_with_same_dset), config["test_samples_per_client"], is_train=False)
           
            train_indices = test_idx_split[self.node_id-1]
            test_dset = Subset(test_dset, train_indices)
        else:
            raise ValueError("Unknown test label distribution: {}.".format(config["test_label_distribution"]))
    ...
    def print_data_summary(self, train_test, test_dset, val_dset=None):
        ...
        print("Train samples per class: {}".format(sorted(train_sample_per_class.items())))
        if val_dset is not None:
            print("Val samples per class: {}".format(sorted(val_sample_per_class.items())))
        print("Test samples per class: {}".format(sorted(test_sample_per_class.items())))
...
class BaseFedAvgClient(BaseClient):
    ...
    def weighted_aggregate(self, models_wts:Dict[int, OrderedDict[str, Tensor]], collab_weights_dict:Dict[int, float], keys_to_ignore=[], 
        ...
        models_coeffs = [(id, models_wts[id], collab_weights_dict[id]) for id in selected_collab]
        
        last_layer_keys = []
        if label_dict is not None:
            last_layer_keys = [key for key in self.model_utils.get_last_layer_keys(models_wts[self.node_id])]
        ...
        if label_dict is not None:
            ...
            if len(last_layer_weight_key) != 1 and len(last_layer_bias_key) != 1:
                raise ValueError("Unsupported last layer format, expected one weights layer and one bias.")        
            ...
            labels_coeff_sum = {host_idx:0.0 for host_idx in label_dict[self.node_id].values()}
            
            # Match and aggregate weights of the last layer based on each node's label dictionary
            host_model = models_wts[self.node_id]
            agg_wts[last_layer_weight_key] = torch.zeros_like(host_model[last_layer_weight_key])
            for id, ext_model, coeff in models_coeffs:
                ...
                labels_map = [(ext_idx, host_label_to_idx[ext_label]) for ext_label, ext_idx in ext_label_to_idx.items() if ext_label in host_label_to_idx]
    ...
    def get_collaborator_weights(self, reprs_dict:Dict[int, OrderedDict[int, Tensor]]) -> Dict[int, float]:
        raise NotImplementedError
