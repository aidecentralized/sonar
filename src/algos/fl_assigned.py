...
class FedAssClient(BaseFedAvgClient):
    ...
    def get_collaborator_weights(self, round):
        ...
        if self.config["strategy"] =="fixed":
            collab_weights = {id: 1 for id in self.config["assigned_collaborators"][self.node_id]}
        elif self.config["strategy"] =="direct_expo":
            power = round % math.floor(math.log2(self.config["num_clients"]-1))
            steps = math.pow(2,power)
            collab_id  = int(((self.node_id + steps) % self.config["num_clients"]) + 1)
