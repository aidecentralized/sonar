...
class RandomTopology():
    def get_selected_ids(node_id, config, reprs_dict, communities):
        ...
        if  random.random() <= within_community_sampling or len(communities) == 1:
            ...
            indices = [id for id in sorted(list(reprs_dict.keys())) if id in communities[node_id]]
        else:
            # Consider clients from other communities
            indices = [id for id in sorted(list(reprs_dict.keys())) if id not in communities[node_id]]      
              
        num_clients_to_select = config[f"target_clients_{'before' if round < config['T_0'] else 'after'}_T_0"]
