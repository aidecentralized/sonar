...
class FedDataRepClient(BaseFedAvgClient):
    ...
    def get_second_step_representation(self, reprs_dict):
        ...
        if self.config["similarity_metric"] in [
            ...
            # LTLR is contained in CTCR from the point of view of the client
    ...
    def get_collaborators_similarity(
        ...
        if self.config["similarity_metric"] == "CTCR_KL":
            ...
            self.config["similarity_metric"] == "log_all_metrics"
        ):  # Should be combined
    ...
    def run_protocol(self):
        ...
        for round in range(start_round, total_rounds):
            ...
            reprs = self.comm_utils.wait_for_signal(
                src=self.server_node, tag=self.tag.REPS1_SHARE
            )
            ...
            if self.require_second_step:
                ...
                second_step_reprs_dict = self.comm_utils.wait_for_signal(
                    src=self.server_node, tag=self.tag.REPS2_SHARE
                )
            ...
            if self.with_sim_consensus:
                ...
                similarity_dict = self.comm_utils.wait_for_signal(
                    src=self.server_node, tag=self.tag.SIM_SHARE
                )

            is_num_collab_changed = (
                round == self.config["T_0"]
                and self.config["target_clients_after_T_0"]
                != self.config["target_clients_before_T_0"]
            )
            is_selection_round = (
                round == start_round
            ...
            if is_selection_round:
                ...
                if num_collaborator_before == 0 and is_before_T0:
                    # Run selection to log mock selection etc (if not must
                    # handle change in server communication)
                    self.select_top_k(
                        similarity_dict, num_collaborator_after, round, total_rounds
                    )
            ...
            models_wts = self.comm_utils.wait_for_signal(
