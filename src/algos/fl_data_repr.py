...
TWO_STEP_STRAT= ["CTAR_KL", "LTLR_KL", "CTLR_KL",
                 "train_loss_inv", "train_loss_sm",
                 "euclidean_pairwise_KL",
                 "dist_corr_AR", "dist_corr_LR",
                 "log_all_metrics"]
CONS_STEP = ["vote_1hop"]

SIM_SHARING = ["affinity_propagation_clustering", "mean_shift_clustering",
               "club","sim_of_sim", "sim_averaging"]
...
class FedDataRepClient(BaseFedAvgClient):
    ...
    def get_dream_reprs(self, num_samples):
        ...
        if self.config["reprs_position"] == 0:
            ...
            reprs_shape = [num_samples] + [max((pos-1) * 64, 64), int(self.config["inp_shape"][1]/2**(max(pos-2, 0))), int(self.config["inp_shape"][2]/2**(max(pos-2, 0)))]
        ...
        for epoch in range(self.config["inv_epochs"] + 1):
            ...
            loss = (
                ...
                +  self.config["alpha_tv"] * total_variation_loss(inputs)#.to(entropy.device)
                + self.config["alpha_l2"] * torch.linalg.norm(inputs)#.to(entropy.device)
    ...
    def get_collaborators_similarity(self, round, reprs_dict, second_step_reprs_dict=None):
        ...
        if sim_metric == "CTCR_KL":
            ...
            sim_dict = self.compute_euclidean_sim_on_top_sim_profile(second_step_reprs_dict)
        ...
        if num_round_avg > 0:
            ...
            if self.node_id == 1:
                print(f"Is round {round} excluded from similarity averaging ? {exclude_round}")
            ...
            for k,v in sim_dict.items():   
                if not exclude_round:
                    ...
                    if self.node_id == 1 and k==1:
                        print(f"Similarity average based on {len(self.running_average[k])} rounds ")
    ...
    def compute_training_loss_similarity(self, second_step_reprs_dict, sim_type):
        ...
        loss_dict = {id: self.loss_fn(softlabels, self.reprs_y).item() for id, softlabels in sl_clients_on_own_data.items()}
    ...
    def select_collaborator(self, collab_similarity, k, round, total_rounds):
        ...
        collab_similarity = {key: value for key, value in collab_similarity.items() if value > 0 and key != self.node_id}
        ...
        if k==0:
            ...
            if strategy == "highest":
                ...
                if strategy == "sim_sampling":
                    total = sum(collab_similarity.values())
                    proba_dist = {key: sim/total for key, sim in collab_similarity.items()}
                ...
                selected_collab = list(np.random.choice(top_x_collab, num_collab, replace=False))
            elif strategy == "eps_greedy":
                eps = self.config.get("eps_greedy", 0.1)
                if np.random.rand() < eps:
                    collab = [id for id in range(1, self.config["num_users"]+1) if id != self.node_id]
                ...
                proba_dist = {key: 1 if key == selected_collab[0] else 0 for key in sorted_collab}
            elif strategy == "uniform_rdm":
                selected_collab = list(np.random.choice(list(collab_similarity.keys()), k, replace=False))
                proba_dist = {key: 1/k for key in selected_collab}
            else:
                raise ValueError("Selection strategy {} not implemented".format(strategy))
        ...
        collab_weights = {key: 1/len(selected_collab) if key in selected_collab else 0 for key in range(1, self.config["num_users"]+1)}
        return collab_weights, proba_dist
    ...
    def get_consensus_similarity(self, collab_similarity, num_client_to_select):
        ...
        if consensus_strategy == "sim_averaging":
            ...
            collab_similarity = self.cluster_points(collab_similarity, num_client_to_select, consensus_strategy)
        elif consensus_strategy == "sim_of_sim":
            collab_similarity = self.compute_euclidean_sim_on_top_sim_profile(collab_similarity)
            ...
            raise ValueError("Consensus strategy {} not implemented".format(consensus_strategy))
    ...
    def weighted_sim_averaging(self, collab_similarity):
        if self.config["similarity_metric"] == "train_loss_inv":
            ...
            # Keep only k highest similarity
            top_a_averaging =self.config.get("sim_consensus_top_a", self.config["num_users"]-1)
            if top_a_averaging < self.config["num_users"]-1: 
    ...
    def vote_consensus(self, collab_similarity):
        ...
        sorted_collab = [id for id, _ in sorted(collab_similarity.items(), key=lambda item: item[1], reverse=True) if id != self.node_id]
        ...
        collab_similarity = {key: value/sum_vote for key, value in candidate_count.items()}
        return collab_similarity
    ...
    def club_consensus(self, collab_similarity):
        ...
        def accept_client(requester, member):
            return requester in sorted_clients[member:,:self.config["club_k_accepted"]]
        ...
        if sorted_clients[own_idx, 0] in own_cluster:
            ...
            return {id+1: 1 if id in own_cluster else 0 for id in collab_similarity.keys()}
            ...
            if weak_link_strategy == "own_cluster_and_pointing_to":
                ...
                raise ValueError("Weak link strategy {} not implemented".format(weak_link_strategy))
    ...
    def cluster_points(self, collab_similarity, k, method):
        ...
        if method == "affinity_propagation_clustering":
            ...
            # Compute clusters
            affinity = 'precomputed' if self.config["affinity_precomputed"] else 'euclidean'
            clustering = AffinityPropagation(random_state=5, affinity=affinity).fit(collab_sim)
        ...
        if len(potential_collab) < 1:
            ...
            print("Client {} has no cluster, found {} closest instead".format(self.node_id, len(potential_collab)))
           
        # Shift index to client id
        potential_collab = [id+1 for id in potential_collab]
            
        # TODO Instead of 1 return similarity with leader/cluster
        sim_dict = {id: 1 if id in potential_collab else 0 for id in collab_similarity.keys()}
    ...
    def compute_euclidean_sim_on_top_sim_profile(self, similarity_profile_per_client):
        ...
        sim_dict = {id+1: 1-euclidean_dist[id]/euclidean_dist.sum() for id in range(len(clients_ids))}
        ...
        for round in range(start_round, total_rounds):
            ...
            if is_selection_round:
                ...
                if num_collaborator_before == 0 and is_before_T0:
                    ...
                    num_collab = num_collaborator_before if is_before_T0 else num_collaborator_after
                    similarity_dict = self.get_consensus_similarity(similarity_dict, num_collab)
                    collab_weights_dict, proba_dist = self.select_collaborator(similarity_dict, num_collab, round, total_rounds)
